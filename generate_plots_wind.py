...
def load_loss(path): 
    ...
    for file in files: 
        ...
        split_id = tokens[-3]
        site_id = tokens[-1]
        df = pd.read_csv(file, index_col=[0,1], header=[0])        
        dfs_split.append((split_id, site_id, df))
    return dfs_split
...
if __name__ == '__main__':
    ...
    loss_mean_valid = pd.DataFrame([{'split': split_id, 
                                     'site': site_id, 
                                     'loss': df.mean().mean()} 
                                    for split_id, site_id, df in dfs_loss_split_valid])
    loss_mean_valid= loss_mean_valid.groupby("split").agg({"loss": "mean"})["loss"].tolist()
